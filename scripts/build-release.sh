...
build_one() {
  local goarch="$1"
  local output="${DIST_DIR}/xpayincus-agent-linux-${goarch}"
...
AMD64_GZ="${DIST_DIR}/xpayincus-agent-linux-amd64.gz"
ARM64_GZ="${DIST_DIR}/xpayincus-agent-linux-arm64.gz"
cat > "${DIST_DIR}/manifest.json" <<EOF_MANIFEST
{
  ...
  "files": {
    "linux-amd64": {
      "name": "xpayincus-agent-linux-amd64.gz",
      "sha256": "$(sha256_file "${AMD64_GZ}")",
      "size": $(size_file "${AMD64_GZ}"),
      "gzip": true
    },
    "linux-arm64": {
      "name": "xpayincus-agent-linux-arm64.gz",
