...
```json
{
  "upgrade": {
    "available": true,
    "version": "v1.0.1",
    "url": "https://<panel>/api/agent/binary/xpayincus-agent-v1.0.1-linux-amd64?v=v1.0.1",
    "sha256": "<sha256>",
    "gzip": false
  }
}
...
Agent 版本统一从 `VERSION` 读取，格式固定为 `vMAJOR.MINOR.PATCH`。
需要发布新版 Agent 时，先递增 `VERSION`，再构建 release 产物。

产物：

```text
dist/xpayincus-agent-v1.0.1-linux-amd64
dist/xpayincus-agent-v1.0.1-linux-arm64
dist/manifest.json
```

`dist` 是本地临时构建目录，不再纳入 Git。正式发布由 GitHub Actions `Agent Build & Release` 完成。

推送中只要 `VERSION` 发生变化，Actions 会读取该版本号，构建并发布 GitHub Release：

```text
tag: agent-v0.0.1
assets:
  xpayincus-agent-v1.0.1-linux-amd64
  xpayincus-agent-v1.0.1-linux-arm64
```

面板运行时不会读取本地 `dist`。它会从 GitHub Release 查询最新 Agent 版本，动态生成 `/api/agent/manifest.json`，并通过 `/api/agent/binary/*` 代理下载对应 Release 资产。

默认 GitHub Release 仓库为 `XiaoLong-Taiwan/XPayincus-Agent`。如果部署到 fork 或私有仓库，可设置：
...
```text
https://<panel>/api/agent/binary/xpayincus-agent-v1.0.1-linux-amd64
https://<panel>/api/agent/binary/xpayincus-agent-v1.0.1-linux-arm64
```
